    ...
    if ( inverter->ReadState() )
      {
        ...
        ESP_LOGD("Aurora", "TransmissionState sensor is: %s", buf); TransmissionState_sensor->publish_state(buf);

        buf[0] = '\0';str->GlobalState(inverter->State.GlobalState).toCharArray(buf, sizeof(buf));
        ESP_LOGD("Aurora", "GlobalState sensor is: %s", buf); GlobalState_sensor->publish_state(buf);

        buf[0] = '\0';str->InverterState(inverter->State.InverterState).toCharArray(buf, sizeof(buf));
        ESP_LOGD("Aurora", "InverterState sensor is: %s", buf); InverterState_sensor->publish_state(buf);

        buf[0] = '\0';str->AlarmState(inverter->State.AlarmState).toCharArray(buf, sizeof(buf));
        ESP_LOGD("Aurora", "AlarmState sensor is: %s", buf); AlarmState_sensor->publish_state(buf);
      }
    if ( inverter->ReadFirmwareRelease() )
      {
        buf[0] = '\0';(inverter->FirmwareRelease.Release).toCharArray(buf, sizeof(buf));
        ESP_LOGD("Aurora", "Aurora Firmware is: %s", buf); Firmware_sensor->publish_state(buf);
      }
  }
};
